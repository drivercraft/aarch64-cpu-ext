...
fn dcache_level(op: CacheOp, level: u64) {
    ...
    let a = 32 - associativity.leading_zeros();  // Log2 of associativity (rounded up)
    
    let way_shift = 32 - a;  // Way field starts at bit (32-A)
    let set_shift = l;       // Set field starts at bit L
    ...
    for set in 0..num_sets {
        for way in 0..associativity {
            ...
            // Way field: bits[31:32-A] 
            // Set field: bits[B-1:L]
            // Note: set_way occupies bits [31:4], level occupies bits [3:1]
